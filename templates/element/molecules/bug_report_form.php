...
function bugReportForm() {
    const appVersion = <?= json_encode($appVersion) ?>;
    ...
    const defaultDescription = `## System-Information
...
`;
    ...
    return {
        ...
        init() {
            // Focus description textarea when modal opens
            this.$nextTick(() => {
        ...
        async submitReport() {
            ...
            try {
                const response = await fetch('<?= $this->Url->build('/bug-reports/add') ?>', {
                    method: 'POST',
                    headers: {
