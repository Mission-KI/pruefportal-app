...
$title_for_layout = __('Process') . ': ' . $process->title . ' - ' . __('VCIO-Einstufung abschließen');
...
<div class="complete-wrapper max-w-4xl mx-auto py-8">
    ...
    <?= $this->element('molecules/primary_card', [
        'title' => __('VCIO-Einstufung abschließen'),
        'subtitle' => $process->title,
        'body' => __('Bitte überprüfen Sie Ihre Angaben vor dem Abschluss der Einstufung. Sie können einzelne Qualitätsdimensionen bei Bedarf noch bearbeiten.')
    ]) ?>

    <!-- 2. Indicators Review Section -->
    ...
    <?= $this->Form->create(null, [
        'url' => ['action' => 'complete', $process->id],
        '@submit.prevent' => 'validateAndSubmit($event)',
        'data-no-loading' => true,
        'data-validation-message' => __("Die VCIO-Einstufung erfüllt den erfassten Schutzbedarf nicht im erforderlichen Maße."),
        'x-data' => '{
    ...
    <div class="final-confirmation mt-8 bg-white rounded-lg shadow p-6">
        <div class="mb-4">
            <?= $this->element('atoms/form_checkbox', [
                ...
                'label' => __('Die VCIO-Einstufung ist vollständig und korrekt ausgefüllt'),
                'attributes' => [
        ...
        <p class="text-gray-600 mb-6">
            <?= __('Nach Abschluss wird die Einstufung an den Prüfer weitergeleitet.') ?>
        </p>

        <?= $this->Form->button(__('Einstufung abschließen'), [
            'type' => 'submit',
